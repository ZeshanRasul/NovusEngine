...
bool CommandBuffer::init(VkRenderData renderData, VkCommandBuffer& commandBuffer) {
  ...
  VkResult result = vkAllocateCommandBuffers(renderData.rdDevice, &allocInfo, &commandBuffer);
...
bool CommandBuffer::submitSingleShotBuffer(VkRenderData renderData, VkCommandBuffer commandBuffer, VkQueue queue) {
  ...
  result = vkCreateFence(renderData.rdDevice, &fenceInfo, nullptr, &bufferFence);
  if (result != VK_SUCCESS) {
    Logger::log(1, "%s error: failed to create buffer fence (error: %i)\n", __FUNCTION__, result);
    return false;
  }

  result = vkResetFences(renderData.rdDevice, 1, &bufferFence);
  if (result != VK_SUCCESS) {
  ...
  result = vkWaitForFences(renderData.rdDevice, 1, &bufferFence, VK_TRUE, UINT64_MAX);
  if (result != VK_SUCCESS) {
    Logger::log(1, "%s error: waiting for buffer fence failed (error: %i)\n", __FUNCTION__, result);
    return false;
  }

  vkDestroyFence(renderData.rdDevice, bufferFence, nullptr);
...
void CommandBuffer::cleanup(VkRenderData renderData, VkCommandBuffer commandBuffer) {
  vkFreeCommandBuffers(renderData.rdDevice, renderData.rdCommandPool, 1, &commandBuffer);
}
