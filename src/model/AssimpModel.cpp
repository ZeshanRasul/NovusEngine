...
bool AssimpModel::loadModel(VkRenderData &renderData, std::string modelFilename, unsigned int extraImportFlags) {
  ...
  if (scene->HasTextures()) {
    unsigned int numTextures = scene->mNumTextures;
      ...
      if (!AssimpTexture::loadTexture(renderData, newTex, texName, data, width, height)) {
        return false;
      }
  ...
  if (!AssimpTexture::loadTexture(renderData, mWhiteTexture, whiteTexName)) {
    Logger::log(1, "%s warning: could not load white default texture '%s', using vertex colors\n", __FUNCTION__, whiteTexName.c_str());
  }

  /* add a placeholder texture in case there is no diffuse tex */
  std::string placeholderTexName = "textures/missing_tex.png";
  if (!AssimpTexture::loadTexture(renderData, mPlaceholderTexture, placeholderTexName)) {
    Logger::log(1, "%s warning: could not load placeholder texture '%s'\n", __FUNCTION__, placeholderTexName.c_str());
  }
...
void AssimpModel::cleanup(VkRenderData &renderData) {
  ...
  for (auto& tex : mTextures) {
    AssimpTexture::cleanup(renderData, tex.second);
  }

  AssimpTexture::cleanup(renderData, mPlaceholderTexture);
  AssimpTexture::cleanup(renderData, mWhiteTexture);
}
