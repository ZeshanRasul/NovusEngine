...
bool AssimpTexture::loadTexture(VkRenderData& renderData, VkTextureData& texData, std::string textureName, aiTexel* textureData, int width, int height, bool generateMipmaps, bool flipImage) {
  if (!textureData) {
...
bool AssimpTexture::uploadToGPU(VkRenderData& renderData, VkTextureData& texData, VkTextureStagingBuffer &stagingData,
  ...
  result = vkCreateImageView(renderData.rdDevice, &texViewInfo, nullptr, &texData.imageView);
  if (result != VK_SUCCESS) {
  ...
  vkGetPhysicalDeviceFeatures(renderData.rdPhysicalDevice, &supportedFeatures);
  ...
  vkGetPhysicalDeviceProperties(renderData.rdPhysicalDevice, &physProperties);
  ...
  result = vkCreateSampler(renderData.rdDevice, &texSamplerInfo, nullptr, &texData.sampler);
  if (result != VK_SUCCESS) {
...
void AssimpTexture::cleanup(VkRenderData &renderData, VkTextureData &texData) {
  if (texData.descriptorSet != VK_NULL_HANDLE) {
    vkFreeDescriptorSets(renderData.rdDevice, renderData.rdDescriptorPool, 1, &texData.descriptorSet);
    texData.descriptorSet = VK_NULL_HANDLE;
  }
  vkDestroySampler(renderData.rdDevice, texData.sampler, nullptr);
  vkDestroyImageView(renderData.rdDevice, texData.imageView, nullptr);
